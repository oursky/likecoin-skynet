...
BINARY_URL=https://github.com/$LIKED_REPOSITORY/releases/download/v$LATEST_VERSION/likecoin-chain_$LATEST_VERSION\_$(uname -s)_$(uname -m).tar.gz

curl -sL $BINARY_URL | tar xz -C /tmp

sudo systemctl stop liked
rm $DAEMON_HOME/cosmovisor/current/bin/liked
cp /tmp/bin/liked $DAEMON_HOME/cosmovisor/current/bin/liked
